...
int main(void)
{
  ...
  spi_nand_read(&sunxi_spi0, SPI_IO_MODE_DUAL_RX, (void *)CONFIG_DTB_LOAD_ADDR, (uint64_t)CONFIG_SPINAND_DTB_ADDR, (uint64_t)sizeof(struct boot_param_header));
  ...
  spi_nand_read(&sunxi_spi0, SPI_IO_MODE_DUAL_RX, (void *)CONFIG_DTB_LOAD_ADDR, (uint64_t)CONFIG_SPINAND_DTB_ADDR, (uint64_t)size);

  /* get kernel size and read */
  spi_nand_read(&sunxi_spi0, SPI_IO_MODE_DUAL_RX, (void *)CONFIG_KERNEL_LOAD_ADDR, (uint64_t)CONFIG_SPINAND_KERNEL_ADDR, (uint64_t)sizeof(struct linux_zimage_header));
  ...
  spi_nand_read(&sunxi_spi0, SPI_IO_MODE_DUAL_RX, (void *)CONFIG_KERNEL_LOAD_ADDR, (uint64_t)CONFIG_SPINAND_KERNEL_ADDR, (uint64_t)size);
